...
solid_geo->transfiniteLine({l4}, 23);
solid_geo->transfiniteLine({l5}, 3);
solid_geo->transfiniteLine({l6}, 23);
...
solid_problem->setDeltat(1.e-3);
solid_problem->setLumpedMass(false);
solid_problem->setSpectralRadius(0.1);
//solid_problem->setGeneralizedAlphas(1.0, 1.0); // In order to get Newmark method with Hu parameters
//solid_problem->setNewmarkParameters(1.0, 1.5); // In order to get Newmark method with Hu parameters

	  ...
	  perm_(nullptr)
{
	...
	int fail2 = system("mkdir -p ./plotData");
...
void SolidDomain::setNewmarkParameters(const double beta, const double gamma)
{
	parameters_->setNewmarkParameters(beta, gamma);
}
...
void SolidDomain::addGraphic(std::string fileName, Variable variable, ConstrainedDOF direction, std::string pointName)
{
	Node *node = geometry_->getPoint(pointName)->getNode();
	outputGraphics_.emplace_back(new OutputGraphic(fileName, variable, direction, node));
}
							   ...
							   const bool &plotMesh, const bool &showInfo)
{
	...
	readInput(pair.first, pair.second, elementType); // cria os nós, elementos e graus de liberdade
	transferGeometricBoundaryConditions();
	transferInitialConditions();
	nodalNeighborSearch();
	elementalNeighborSearch();

	remesh_ = new TriangularMesher;
...
void SolidDomain::solveTransientProblem()
{
	...
	if (parameters_->getInitialAccel())
		computeInitialAccel();
...
double SolidDomain::getSurfaceForcesPotentialEnergy()
{
	double surfaceForcePotentialEnergy;
	double currentTime = parameters_->getCurrentTime();
	for (NeumannBoundaryCondition *const &stnbc : neumannBoundaryConditions_)
		stnbc->getPotentialEnergy(dimension_, currentTime, surfaceForcePotentialEnergy);

	return surfaceForcePotentialEnergy;
}
...
void SolidDomain::exportGraphicData(const int &timeStep)
{
	// Implemented only for 2D problems

	double deltat = parameters_->getDeltat();

	for (auto &outputGraphic : outputGraphics_)
	{
		Node *node = outputGraphic->getNode();
		Variable variable = outputGraphic->getVariable();
		ConstrainedDOF direction = outputGraphic->getConstrainedDOF();
		unsigned int dir;
		if (direction == X)
			dir = 0;
		if (direction == Y)
		...
		std::string name = outputGraphic->getFileName();
		std::string filePath = "plotData/" + name + ".dat";
		...
		if (!file.fail())
		{
			...
			file.width(20);
			file << std::left << deltat * double(timeStep);
			file.width(20);
			if (variable == DISPLACEMENT)
				file << std::left << node->getDegreeOfFreedom(dir)->getCurrentValue() - node->getDegreeOfFreedom(dir)->getInitialValue();
			if (variable == VELOCITY)
				file << std::left << node->getDegreeOfFreedom(dir)->getCurrentFirstTimeDerivative();
			if (variable == CAUCHY_STRESS)
			{
				double *cauchyStress = node->getCauchyStress();
				file << std::left << cauchyStress[dir];
			}
			file << "\n";
			file.close();
		}
	...
	double totalKinectEnergy = 0.0;
	double totalExternalPotentialEnergy = 0.0;
	for (Element *&el : elements_)
	{
		double strainEnergy, kinectEnergy, domainForcePotentialEnergy;
		el->getEnergy(strainEnergy, kinectEnergy, domainForcePotentialEnergy);
		totalStrainEnergy += strainEnergy;
		totalKinectEnergy += kinectEnergy;
		totalExternalPotentialEnergy += domainForcePotentialEnergy;
	}
	double surfaceForcePotentialEnergy = getSurfaceForcesPotentialEnergy();
	totalExternalPotentialEnergy += surfaceForcePotentialEnergy;

	double totalEnergy = totalStrainEnergy + totalKinectEnergy - totalExternalPotentialEnergy;

	// std::cout << "\nDeformation energy = " << totalDeformationEnergy << std::endl;
	// std::cout << "Kinect energy = " << totalKinectEnergy << std::endl;
	// std::cout << "External potential energy = " << totalExternalPotentialEnergy << std::endl;
	// std::cout << "Total energy = " << totalEnergy << std::endl;

	// Strain Energy
	std::ofstream file1("plotData/strain-energy.dat", std::ios::app);
	if (!file1.fail())
	{
		file1.precision(8);
		file1.setf(std::ios::scientific, std::ios::floatfield);
		file1.width(20);
		file1 << std::left << deltat * double(timeStep);
		file1.width(20);
		file1 << std::left << totalStrainEnergy;
		file1 << "\n";
		file1.close();
	}
	else
	{
		file1.close();
		std::cerr << "\nCan't open the file '" << "plotData/strain-energy.dat" << "'.\n";
		exit(EXIT_FAILURE);
	}

	// Kinect Energy
	std::ofstream file2("plotData/kinect-energy.dat", std::ios::app);
	if (!file2.fail())
	{
		file2.precision(8);
		file2.setf(std::ios::scientific, std::ios::floatfield);
		file2.width(20);
		file2 << std::left << deltat * double(timeStep);
		file2.width(20);
		file2 << std::left << totalKinectEnergy;
		file2 << "\n";
		file2.close();
	}
	else
	{
		file2.close();
		std::cerr << "\nCan't open the file '" << "plotData/kinect-energy.dat" << "'.\n";
		exit(EXIT_FAILURE);
	}

	// Potential External Energy
	std::ofstream file3("plotData/external-energy.dat", std::ios::app);
	if (!file3.fail())
	{
		file3.precision(8);
		file3.setf(std::ios::scientific, std::ios::floatfield);
		file3.width(20);
		file3 << std::left << deltat * double(timeStep);
		file3.width(20);
		file3 << std::left << -totalExternalPotentialEnergy;
		file3 << "\n";
		file3.close();
	}
	else
	{
		file3.close();
		std::cerr << "\nCan't open the file '" << "plotData/external-energy.dat" << "'.\n";
		exit(EXIT_FAILURE);
	}

	// Total Energy
	std::ofstream file4("plotData/total-energy.dat", std::ios::app);
	if (!file4.fail())
	{
		file4.precision(8);
		file4.setf(std::ios::scientific, std::ios::floatfield);
		file4.width(20);
		file4 << std::left << deltat * double(timeStep);
		file4.width(20);
		file4 << std::left << totalEnergy;
		file4 << "\n";
		file4.close();
	}
	else
...
void SolidDomain::exportToParaview(const int &step)
{
	...
	if (materials_[0]->getType() == MaterialType::ELASTIC_INCOMPRESSIBLE_SOLID ||
		materials_[0]->getType() == MaterialType::NEWTONIAN_INCOMPRESSIBLE_FLUID)
		mixed = true;
...
void SolidDomain::readInput(const std::string &inputFile, const bool &deleteFiles, const PartitionOfUnity elementType)
{
	...
	const std::unordered_map<int, ParametricElement *> gmshElements =
		{
			// lines
			{1, &ParametricLineElement::L2},
			{8, &ParametricLineElement::L3},
			{26, &ParametricLineElement::L4},
			// triangles
			{2, &ParametricSurfaceElement::T3},
			{9, &ParametricSurfaceElement::T6},
			{21, &ParametricSurfaceElement::T10},
			// quadrilaterals
			{3, &ParametricSurfaceElement::Q4},
			{10, &ParametricSurfaceElement::Q9},
			{36, &ParametricSurfaceElement::Q16}};
	...
	unsigned int nPoints = nElements - nLineElements - nSurfaceElements;
...
void SolidDomain::transferInitialConditions()
{
	auto start_timer = std::chrono::high_resolution_clock::now();

	// Transfering initial conditions
	for (auto &inic : geometry_->getInitialConditions())
	{
		// Transfering nodal displacements and velocities applied to points
		if (inic->getPoint())
		{
			Point *p = inic->getPoint();
			Node *n = p->getNode();
			double val = inic->getValue();
			int ndofs = n->getNumberOfDegreesOfFreedom();
			ConstrainedDOF constrainedDOF = inic->getDegreeOfFreedom();
			Variable variable = inic->getVariable();
			switch (variable)
			{
			case Variable::DISPLACEMENT:
			{
				switch (constrainedDOF)
				{
				case ConstrainedDOF::X:
				{
					DegreeOfFreedom *dof = n->getDegreeOfFreedom(0);
					initialConditions_.first.emplace_back(new InitialCondition(n, dof, dof->getPastValue() + val));
					break;
				}
				case ConstrainedDOF::Y:
				{
					DegreeOfFreedom *dof = n->getDegreeOfFreedom(1);
					initialConditions_.first.emplace_back(new InitialCondition(n, dof, dof->getPastValue() + val));
					break;
				}
				case ConstrainedDOF::Z:
				{
					DegreeOfFreedom *dof = n->getDegreeOfFreedom(2);
					initialConditions_.first.emplace_back(new InitialCondition(n, dof, dof->getPastValue() + val));
					break;
				}
				case ConstrainedDOF::ALL:
				{
					for (int i = 0; i < ndofs; i++)
					{
						DegreeOfFreedom *dof = n->getDegreeOfFreedom(i);
			...
			case Variable::VELOCITY:
			{
				...
				case ConstrainedDOF::X:
				{
					...
					DegreeOfFreedom *dof = n->getDegreeOfFreedom(0);
					initialConditions_.second.emplace_back(new InitialCondition(n, dof, val));
					break;
				}
				case ConstrainedDOF::Y:
				{
		...
		// Transfering nodal initial displacements and velocities applied to lines
		else if (inic->getLine())
		{
			Line *l = inic->getLine();
			double val = inic->getValue();
			ConstrainedDOF constrainedDOF = inic->getDegreeOfFreedom();
		...
		// Transfering nodal initial displacements and velocities applied to surfaces
		else if (inic->getSurface())
		{
			Surface *s = inic->getSurface();
			double val = inic->getValue();
			ConstrainedDOF constrainedDOF = inic->getDegreeOfFreedom();
			Variable variable = inic->getVariable();
			const std::vector<Node *> &nodes = s->getNodes();
...
void SolidDomain::domainDecompositionMETIS(const PartitionOfUnity &elementType)
{
	...
	idx_t *elem_start = new idx_t[numEl + 1];
	idx_t *elem_connec = new idx_t[n * numEl];
...
void SolidDomain::reorderDOFs()
{
	...
	if (!perm_)
		perm_ = new idx_t[n]; // array of index permutation according to METIS. perm[i] is the ith node in container
...
void SolidDomain::applyInitialConditions()
{
	// applying displacement initial conditions
	for (auto inic : initialConditions_.first)
	{
		DegreeOfFreedom *dof = inic->getDegreeOfFreedom();
		dof->setPastValue(inic->getValue());
		dof->setIntermediateValue(inic->getValue());
		dof->setCurrentValue(inic->getValue());
	}
	// applying velocities initial conditions
	for (auto inic : initialConditions_.second)
	{
		DegreeOfFreedom *dof = inic->getDegreeOfFreedom();
		dof->setPastFirstTimeDerivative(inic->getValue());
		dof->setIntermediateFirstTimeDerivative(inic->getValue());
		dof->setCurrentFirstTimeDerivative(inic->getValue());
	}
}
...
void SolidDomain::printNodalSolution(Node *&node, std::string messege)
{
	// std::cout << std::setprecision(8) << std::fixed;
	std::cout << messege << std::endl;
	int ndofs = node->getNumberOfDegreesOfFreedom();
	std::cout << "ndofs = " << ndofs << std::endl;
	for (int i = 0; i < ndofs; i++)
	{
		DegreeOfFreedom *dof = node->getDegreeOfFreedom(i);
		int index = dof->getIndex();
		if (dof->getType() == DOFType::POSITION)
			std::cout << "Posição #" << index << " = " << dof->getCurrentValue() << std::endl;
		if (dof->getType() == DOFType::VELOCITY)
			std::cout << "Velocidade #" << index << " = " << dof->getCurrentValue() << std::endl;
		if (dof->getType() == DOFType::CONTACT_FORCE)
			std::cout << "Força de contato #" << index << " = " << dof->getCurrentValue() << std::endl;
		if (dof->getType() == DOFType::CONTACT_MOMENTUM)
			std::cout << "Momentum de contato #" << index << " = " << dof->getCurrentValue() << std::endl;
	}
}
...
void ParametricLineElement::getShapeFunctionsSecondDerivatives(double* xsi, double**& d2phi_dxsi2) const
{
    d2phi_dxsi2 = new double*;
    d2phi_dxsi2[0] = new double[order_ + 1];
    double xsi1 = xsi[0];
    ...
    case PartitionOfUnity::L3:
        ...
        break;
    case PartitionOfUnity::L4:
        d2phi_dxsi2[0][0] = -9.0/16.0  * (2.0 * xsi1 * (xsi1 - 1.0) + (xsi1*xsi1 - 1.0/9.0));
        d2phi_dxsi2[0][1] =  9.0/16.0  * (2.0 * xsi1 * (xsi1 + 1.0) + (xsi1*xsi1 - 1.0/9.0));
        d2phi_dxsi2[0][2] = 27.0/16.0 * (2.0 * xsi1 * (xsi1 - 1.0/3.0) + (xsi1 * xsi1 - 1.0));
        d2phi_dxsi2[0][3] = -27.0/16.0 * (2.0 * xsi1 * (xsi1 + 1.0 / 3.0) + (xsi1 * xsi1 - 1.0));
        break;
    default:
        break;
    }

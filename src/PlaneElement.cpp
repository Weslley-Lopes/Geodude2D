...
PlaneElement::PlaneElement(const int &index,
                           const std::vector<DegreeOfFreedom *> &degreesOfFreedoms,
                           Material *material,
                           BaseSurfaceElement *base,
                           AnalysisParameters *params)
    : Element(index, degreesOfFreedoms),
      material_(material),
      base_(base),
      parameters_(params)
{
...
AnalysisParameters *PlaneElement::getAnalysisParameters()
{
    return parameters_;
}
    ...
    for (auto &qp : quadraturePoints)
    {
        ...
        { // Pressure degrees of freedom
            for (int i = 0; i < ndofs2; i++)
            {
                ...
                // pspg mass part
                double m_pspg = factor7 * (dphi_dy[a][0] * accel[0] + dphi_dy[a][1] * accel[1]);
                m_pspg = 0.0;
...
inline void PlaneElement::getCurrentDeformationGradient(double dphi_dx[][2],
                                                        double A[2][2]) const
{
    A[0][0] = 0.0;
    A[0][1] = 0.0;
    ...
    int numberOfNodes = base_->getNumberOfNodes();
    int ndofs = 2 * numberOfNodes;
    ...
    for (int a = 0; a < ndofs; a++)
    {
        ...
        // dir j
        int j = a % 2;
        double y2 = degreesOfFreedom_[a]->getCurrentValue();

        A[j][0] += dphi_dx[i][0] * y2;
        A[j][1] += dphi_dx[i][1] * y2;
    }
}
...
void PlaneElement::getEnergy(double &deformationEnergy,
                             double &kinectEnergy,
                             double &domainForcePotentialEnergy) const
{
    // std::cout << "====================================================================================" << std::endl;
    // std::cout << "Element index: " << base_->getIndex() << std::endl;
    // std::cout << "====================================================================================" << std::endl;

    unsigned int ndofs = degreesOfFreedom_.size();
    const std::vector<Node *> &nodes = base_->getNodes();
    const unsigned int numberOfNodes = nodes.size();

    const std::vector<QuadraturePoint *> &spatialQuadraturePoints = base_->getParametricElement()->getQuadraturePoints();
    ...
    kinectEnergy = 0.0;
    domainForcePotentialEnergy = 0.0;

    for (auto &sqp : spatialQuadraturePoints)
    {
        ...
        double dy_dx[2][2];
        getCurrentDeformationGradient(dphi_dx, dy_dx);
        ...
        double dE_dy[ndofs][3];
        getStrainTensorFirstDerivative(dy_dx, dphi_dx, dE_dy);

        double E[3];
        double S[3];
        double dS_dy[ndofs][3];

        switch (material_->getType())
        {
        case MaterialType::ELASTIC_SOLID:
            getStrainTensor(dy_dx, E);
            material_->getPlaneStressTensorAndDerivative(ndofs, E, dE_dy, nullptr, S, dS_dy);
            break;
        }

        const double factor = spaceWeight * j0;

        // Deformation energy
        deformationEnergy += 0.5 * doubleContraction(S, E) * factor;
        ...
        for (int a = 0; a < ndofs; a++)
        {
            ...
            double y2A = degreesOfFreedom_[a]->getCurrentValue();               // current position
            double v2A = degreesOfFreedom_[a]->getCurrentFirstTimeDerivative(); // current velocity

            // External potential energy due to domain forces
            domainForcePotentialEnergy += density * gravity[j] * phi[i] * y2A * factor;
            ...
            for (int b = 0; b < ndofs; b++)
            {
                ...
                double y2B = degreesOfFreedom_[b]->getCurrentValue();               // current position
                double v2B = degreesOfFreedom_[b]->getCurrentFirstTimeDerivative(); // current velocity

                // Kinect energy
                if (j == l)
                {
                    kinectEnergy += 0.5 * density * phi[k] * v2B * phi[i] * v2A * factor;
                    // kinectEnergy += density * phi[k] * at * phi[i] * yt * factor;
                    // std::cout << "\nDensity = " << density << std::endl;
                    // std::cout << "Phi[" << k << "] k = " << phi[k] << std::endl;
                    // std::cout << "Phi[" << i << "] i = " << phi[i] << std::endl;
    ...
    // std::cout << "Deformation energy = " << deformationEnergy << std::endl;
    // std::cout << "Kinect energy = " << kinectEnergy << std::endl;
    // std::cout << "External potential energy = " << domainForcePotentialEnergy << std::endl;
}